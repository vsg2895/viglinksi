  ...
  return (
    <section className="mt-10" aria-labelledby="player-reviews">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <h2 id="player-reviews" className="font-display text-2xl font-bold text-ink">
      ...
      {reviews.length === 0 ? (
        <p className="mt-4 text-muted">{COPY.reviews.empty}</p>
      ) : (
        <ul className="mt-6 flex flex-col gap-4">
          {reviews.map((review) => (
            <li key={review.id} className="rounded-2xl border border-line bg-paper p-5">
              <div className="flex flex-wrap items-center gap-3">
                <span className="text-sm font-semibold text-gold" aria-label={`${review.rating} out of 5`}>
                  {'★'.repeat(review.rating)}
                  <span className="text-line-soft">{'★'.repeat(5 - review.rating)}</span>
                </span>
                <span className="text-sm font-semibold text-ink">{review.author_name}</span>
                {review.published_at && (
                  <time dateTime={review.published_at} className="text-xs text-faint">
              ...
              <p className="mt-1 whitespace-pre-line text-sm leading-relaxed text-ink-soft">{review.body}</p>
            </li>
          ))}
        </ul>
      )}
